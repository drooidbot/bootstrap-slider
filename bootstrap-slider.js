...
(function($) {
    ...
    var Slider = function(element, options) {
            ...
            this.picker = $(
                    '<div class="slider">' +
                        ...
                        '<div id="tooltip_max" class="tooltip">' + 
                            ...
                            '<div class="tooltip-inner"></div>' + 
                        '</div>' +
                        '<ul class="ruler unstyled inline"></ul>' +
                    '</div>')
        ...
        switch (this.orientation) {
            ...
            default:
                ...
                this.tooltip_min.addClass('bottom')[0].style.top = -this.tooltip_min.outerHeight() - 14 + 'px';
                this.tooltip_max.addClass('bottom')[0].style.top = -this.tooltip_max.outerHeight() - 14 + 'px';
                break;
        }
        ...
        this.showRuler = options.show_ruler;
        this.ruler = this.picker.find('.ruler');
    ...
    Slider.prototype = {
        ...
        setRuler: function() {
            ...
            var stepPercentages = Math.min(this.percentage[2], 100);
            var noOfSteps = Math.floor(100 / this.percentage[2]);
            
            this.ruler.html('');
            for (var i = 0; i <= noOfSteps; i++) {
                this.ruler.append($('<li></li>').css('left', Math.min((stepPercentages * i), 100) + '%'));
            }
        },
        layout: function() {
            ...
            if (this.range) {
                ...
                this.tooltip[0].style[this.stylePos] = (positionPercentages[1] + positionPercentages[0]) / 3 + '%';
                if (this.orientation === 'vertical') {
                    this.tooltip.css('margin-top', (-this.tooltip.outerHeight() / 2) + 2 + 'px');
                } else {
                    this.tooltip.css('margin-left', (-this.tooltip.outerWidth() / 2) + 2 + 'px');
                }
                ...
                this.tooltipInner_min.html(
                        this.formater(this.value[0])
                        );
                this.tooltipInner_max.html(
                        this.formater(this.value[1])
                        );

                this.tooltip_min[0].style[this.stylePos] = positionPercentages[0] + '%';
                if (this.orientation === 'vertical') {
                    this.tooltip_min.css('margin-top', (-this.tooltip_min.outerHeight() / 2) + 2 + 'px');
                } else {
                    this.tooltip_min.css('margin-left', (-this.tooltip_min.outerWidth() / 2) + 2 + 'px');
                }
                this.tooltip_max[0].style[this.stylePos] = positionPercentages[1] + '%';
                if (this.orientation === 'vertical') {
                    this.tooltip_max.css('margin-top', (-this.tooltip_max.outerHeight() / 2) + 2 + 'px');
                } else {
                    this.tooltip_max.css('margin-left', (-this.tooltip_max.outerWidth() / 2) + 2 + 'px');
                }
            } else {
                this.tooltipInner.html(
                        this.formater(this.value[0])
                ...
                if (this.orientation === 'vertical') {
                    ...
                    //@todo(8-aug-14): do the steps below for other cases (range slider & vertical slider) as well
                    var leftValue = positionPercentages[0] === 0 ? 
                                        -25 : (positionPercentages[0] === 100 ?
                                                -this.tooltipInner.outerWidth() + 25 :
                                                        -this.tooltipInner.outerWidth() / 2);
                                                
                    this.tooltipInner.css('left', leftValue + 'px');
                }
            }
        },
